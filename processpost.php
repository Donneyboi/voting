...
if (1==1) {
	...
	if ($votename=="" and $_FILES['myfile']=="") {
		...
		$insertpost=mysqli_query($conn," insert into name(email,name,time)values('$email','$votename','$time')");
		...
		if ($lastid>0) {
		 if ($_FILES['myfile']=="") {
             ...
             if (move_uploaded_file($filename2, $mainfile)) {
             	$insertpic=mysqli_query($conn,"insert into image(imagename,postid,time)values('$mainfile2','$lastid','$time')");
